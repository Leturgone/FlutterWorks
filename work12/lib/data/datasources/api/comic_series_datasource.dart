...
class ComicSeriesDataSource{
  ...
  final List<ComicSeries> _seriesList = [
    ComicSeries(
        id: 1,
  ...
  Future<ComicSeries> fetchSeries(int id) async{
    await Future.delayed(Duration(seconds: 2)); //// имитация задержки
    return _seriesList.firstWhere(
          (series) => series.id == id,
      orElse: () => throw Exception('Series with id $id not found'),
    );
  }

  Future<List<ComicSeries>> fetchSeriesList() async {
    await Future.delayed(Duration(seconds: 2)); //// имитация задержки
